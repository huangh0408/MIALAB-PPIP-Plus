...
#features dimension: 3
		...
		paste ${i}_${k}.txt ${temp}_${k}_${k}_EC_IC_EV.txt >${temp}_${k}.txt
		sed -i "s/RES/$temp/" ${temp}_${k}.txt
		awk '{print $1,$3,$2,$4,$5,$6,$12,$14,$13,$8,$9,$10,$11}' ${temp}_${k}.txt >${temp}_${k}_temp.txt
		cat ${temp}_${k}_temp.txt |column -t >${temp}_${k}.txt
		cp ${temp}_${k}.txt $main_directory/results/features_by_chain
		rm -f ${temp}_${k}_temp.txt
		rm -f ${temp}_${k}_.txt
...
#create flag
				...
				for((n=1;n<=sumflag;n++));do
					...
					changecol_jl=$(awk '$4=="'"$flagcol_j"'"&&$18=="'"$flagcol_l"'"{print NR}' ${temp}_${j}_${l}_.txt)
					echo $changecol_jl >>${temp}_${j}_${l}_hh.txt
					sed -i "${changecol_jl}s/0/1/" ${temp}_${j}_${l}_flag.txt
				done
				paste ${temp}_${j}_${l}_.txt ${temp}_${j}_${l}_flag.txt >${temp}_${j}_${l}.txt
				sed -i "s/RES/$temp/" ${temp}_${j}_${l}.txt
